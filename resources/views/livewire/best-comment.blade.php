<div>
    <div class="max-w-7xl my-4 mx-auto sm:px-6 lg:px-8">
        <div class="bg-white flex justify-between dark:bg-gray-900 overflow-hidden shadow-sm sm:rounded-lg group relative">
            ...
            @if($thread->user_id == Auth::id())
                <div class="my-auto cursor-pointer" wire:click.prevent="unmarkBest">
                    <span class="bg-red-100 text-red-800 text-sm font-medium px-2.5 py-0.5 rounded-bl rounded-tl dark:bg-red-900 dark:text-red-300 h-6 pb-1 absolute right-2 translate-x-full -translate-y-1/2 transition duration-300 ease-in-out transform group-hover:translate-x-2">Unmark</span>
                </div>
            @endif
        </div>
    </div>
